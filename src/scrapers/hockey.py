"""Hockey scraper for betting sites."""
...
class HockeyScraper(BaseScraper):
    """Scraper for hockey matches from betting sites."""
    
    def __init__(self, site_name: str, sport: str = 'hockey'):
        super().__init__(site_name, sport)
    
    def scrape_matches(self, url: str) -> List[Dict]:
        """Scrape hockey matches from the specified URL."""
        ...
        try:
            if 'tipsport' in self.site_name.lower():
                ...
                scraper_logger.warning(f"Unknown site for hockey scraping: {self.site_name}")
        
        except Exception as e:
            scraper_logger.error(f"Error scraping {self.site_name} hockey: {e}")
        
        return matches
    
    def _scrape_tipsport(self) -> List[Dict]:
        """Scrape hockey matches from Tipsport."""
        matches = []
        ...
        scraper_logger.info(f"Scraped {len(matches)} hockey matches from Tipsport")
        return matches
    ...
    def _extract_tipsport_match(self, match_element) -> Dict:
        ...
        # Extract odds (1X2 format for hockey)
    ...
    def _scrape_fortuna(self) -> List[Dict]:
        """Scrape hockey matches from Fortuna."""
        matches = []
        ...
        scraper_logger.info(f"Scraped {len(matches)} hockey matches from Fortuna")
        return matches
    ...
    def _scrape_nike(self) -> List[Dict]:
        """Scrape hockey matches from Nike."""
        matches = []
        ...
        scraper_logger.info(f"Scraped {len(matches)} hockey matches from Nike")
        return matches
