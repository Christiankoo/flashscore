"""Volleyball scraper for betting sites."""
...
class VolleyballScraper(BaseScraper):
    """Scraper for volleyball matches from betting sites."""
    
    def __init__(self, site_name: str, sport: str = 'volleyball'):
        super().__init__(site_name, sport)
    
    def scrape_matches(self, url: str) -> List[Dict]:
        """Scrape volleyball matches from the specified URL."""
        ...
        try:
            if 'tipsport' in self.site_name.lower():
                ...
                scraper_logger.warning(f"Unknown site for volleyball scraping: {self.site_name}")
        
        except Exception as e:
            scraper_logger.error(f"Error scraping {self.site_name} volleyball: {e}")
        
        return matches
    
    def _scrape_tipsport(self) -> List[Dict]:
        """Scrape volleyball matches from Tipsport."""
        matches = []
        ...
        scraper_logger.info(f"Scraped {len(matches)} volleyball matches from Tipsport")
        return matches
    ...
    def _extract_tipsport_match(self, match_element) -> Dict:
        ...
        # Extract odds (Home/Away format for volleyball)
    ...
    def _scrape_fortuna(self) -> List[Dict]:
        """Scrape volleyball matches from Fortuna."""
        matches = []
        ...
        scraper_logger.info(f"Scraped {len(matches)} volleyball matches from Fortuna")
        return matches
    ...
    def _scrape_nike(self) -> List[Dict]:
        """Scrape volleyball matches from Nike."""
        matches = []
        ...
        scraper_logger.info(f"Scraped {len(matches)} volleyball matches from Nike")
        return matches
